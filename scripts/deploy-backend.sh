...
# Fetch secrets from SSM using JSON to avoid shell interpretation of special chars
...
docker run -d --name starttech-backend --restart always -p 8080:8080 \
